...
class UserNotificationCenter: NSObject {
	...
	func postProxyChangeByOtherAppNotice() {
		post(title: NSLocalizedString("System Proxy Changed", comment: ""),
			 info: NSLocalizedString("Proxy settings are changed by another process. Neko is no longer the default system proxy.", comment: ""), notiOnly: true)
	}
	
	func postUpdateNotice(msg: String) {
		postNotificationAlert(title: "Update Neko Sing-box", info: msg)
	}
}

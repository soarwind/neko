...
class ClashStatusTool {
    static func checkPortConfig(cfg: ClashConfig?) {
        ...
        if cfg.usedHttpPort == 0 {
            ...
            let alert = NSAlert()
            alert.messageText = NSLocalizedString("Neko Start Error!", comment: "")
            alert.informativeText = NSLocalizedString("Ports Open Fail, Please try to restart Neko", comment: "")

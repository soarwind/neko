...
class AppVersionUtil: NSObject {
    ...
    private static let kLastVersionNumberKey = "com.Neko.lastVersionNumber"
...
extension AppVersionUtil {
    static func showUpgradeAlert() {
        if let lastVersion = shared.lastVersionNumber, hasVersionChanged {
            ...
            let alert = NSAlert()
            alert.messageText = NSLocalizedString("This version of Neko contains a break change due to clash core 1.0 released. Check if your config is not working properly.", comment: "")

...
class PrivilegedHelperManager {
    ...
    static let machServiceName = "com.metacubex.Neko.ProxyConfigHelper"
            ...
            case .noFound:
                if #available(macOS 13, *) {
                    ...
                    if status == .requiresApproval {
                        let alert = NSAlert()
                        let notice = NSLocalizedString("Neko use a daemon helper to setup your system proxy. Please enable Neko in the Login Items under the Allow in the Background section and relaunch the app", comment: "")
                        let addition = NSLocalizedString("If you can not find Neko in the settings, you can try reset daemon", comment: "")
...
extension PrivilegedHelperManager {
    ...
    private func showInstallHelperAlert() -> Bool {
        let alert = NSAlert()
        alert.messageText = NSLocalizedString("Neko needs to install/update a helper tool with administrator privileges, otherwise Neko won't be able to configure system proxy.", comment: "")
...
private enum DaemonInstallResult {
    ...
    var alertContent: String {
        ...
        case let .blessError(code):
            ...
            case kSMErrorJobMustBeEnabled: return "Neko Helper is disabled by other process. Please run \"sudo launchctl enable system/\(PrivilegedHelperManager.machServiceName)\" in your terminal. The command has been copied to your pasteboard"

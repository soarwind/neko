...
class ApiRequest {
    ...
    private lazy var logQueue = DispatchQueue(label: "com.Neko.core.log")

    static let clashRequestQueue = DispatchQueue(label: "com.neko.clashRequestQueue")
    ...
    static func requestConfigUpdate(configName: String, callback: @escaping ((ErrorString?) -> Void)) {
        findConfigPath(configName: configName) { path in
            ...
            do {
                ...
                callback("neko_meta_config error \(error)")
                return
            }
    ...
    static func requestConfigUpdate(configPath: String, callback: @escaping ((ErrorString?) -> Void)) {
        let placeHolderErrorDesp = "Error occoured, Please try to fix it by restarting Neko. "

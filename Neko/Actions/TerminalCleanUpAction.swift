...
enum TerminalConfirmAction {
    static func run() -> NSApplication.TerminateReply {
        ...
        if ConfigManager.shared.proxyPortAutoSet && !ConfigManager.shared.isProxySetByOtherVariable.value || NetworkChangeNotifier.isCurrentSystemSetToClash(looser: true) ||
            ...
            Logger.log("Neko quit need clean proxy setting")
        ...
        if !shouldWait {
            Logger.log("Neko quit without clean waiting")
            return .terminateNow
        }
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            switch res {
            case .success:
                Logger.log("Neko quit after clean up finish")
            ...
            case .timedOut:
                Logger.log("Neko quit after clean up timeout")
        ...
        Logger.log("Neko quit wait for clean up")
    ...
    static func confirmAction() -> Bool {
        if NSApp.activationPolicy() == .regular {
            let alert = NSAlert()
            alert.messageText = NSLocalizedString("Quit Neko?", comment: "")
